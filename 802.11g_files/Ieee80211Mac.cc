...
//
// Copyright (C) 2006 Andras Varga and Levente M�z�os
//
...
{
	thStats.recordScalar();
	cwStats.recordScalar();
...
void Ieee80211Mac::initialize(int stage)
{
	...
	if(ST==-1)
		ST=20e-6;//20us
	EV<<" slotTime="<<ST*1e6<<"us DIFS="<<DIFSPeriod()*1e6<<"us";
	...
	EV<<" bitrate="<<bitrate/1e6<<"M IDLE="<<IDLE<<" RECEIVE="<<RECEIVE<<endl;
        ...
        endSIFS = new cMessage("endSIFS");
        endDIFS = new cMessage("endDIFS");
        endBackoff = new cMessage("endBackoff");
        endTimeout = new cMessage("endTimeout");
        endReserve = new cMessage("endReserve");
...
void Ieee80211Mac::handleWithFSM(cMessage *msg)
{
    ...
    else if (isUpperMsg(msg) && fsm.state() != IDLE)
    {
    ...
    FSMA_Switch(fsm)
    {
	EV<<"\nis_event="<<___is_event<<endl;
       /* FSMA_State(INIT){
		FSMA_Event_Transition(Data-Ready,
                                  isUpperMsg(msg),
                                  DEFER,
                		invalidateBackoffPeriod();backoff=true;
            );
...
*/
	...
	*///changed
	FSMA_Event_Transition(Transmit-Broadcast,
		...
		oldT=simTime();
		if(tCycle>=1000.0E-3){
			receiveBroadcastVector.record(numReceivedBroadcast*(msg->length()-62*8)/tCycle/1e6);
			thStats.collect(numReceivedBroadcast*(msg->length()-62*8)/tCycle/1e6);
			tCycle=0.0;
			numReceivedBroadcast=0;
			}
...
{
    return SIFSPeriod() + ((double)AIFSN) * SlotPeriod();
}
...
void Ieee80211Mac::scheduleDIFSPeriod()
{
    ...
    {
        EV << "scheduling DIFS period: "<<simTime()<<"->"<<simTime() + DIFSPeriod()<<endl;
...
Ieee80211DataOrMgmtFrame *Ieee80211Mac::buildBroadcastFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
    ...
    PhyControlInfo *phyControlInfo_old = dynamic_cast<PhyControlInfo *>( frameToSend->controlInfo() );
	...
	*phyControlInfo_new=*phyControlInfo_old;
	//EV<<"PhyControlInfo bitrate "<<phyControlInfo->bitrate()/1e6<<"Mbps txpower "<<phyControlInfo->txpower()<<"mW"<<endl;
...
double Ieee80211Mac::frameDuration(Ieee80211Frame *msg)
{
    ...
    if( ctrl ){
	ev<<"Per frame2 params bitrate "<<ctrl->bitrate()/1e6<<endl;
	duration = frameDuration(msg->length(), ctrl->bitrate());
	delete ctrl;
	return duration;
	}
    else
