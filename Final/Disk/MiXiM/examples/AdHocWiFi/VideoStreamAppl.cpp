...
void VideoStreamAppl::report(int round)
{
	FOR_EACH(Streams,streamIter,streams) { // over all incoming messages
		cout << "Host: " << nodeID << " with " << streamIter->second->incomingMessages.size() << " on stream #" << streamIter->first << " at round " << round << endl;
	}
}
...
void VideoStreamAppl::updateWeight(long unsigned& newRate,long unsigned oldRate,int streamNumber,long unsigned bits) {
	return;
	float weight = ((float) newRate) / oldRate;
	cout << "INIT: " << nodeID << " weight=" << weight << " stream " << streamNumber << " new=" << newRate << " old=" << oldRate << endl;
	if (weight>0.75)
	  return;
	//newRate = 0;
	FOR_EACH(Int2TasksMap,it,nodeTasks) {
	  FOR_EACH_C(TasksCollection,tit,it->second) {
            if ((*tit)->GetRequestNum()==streamNumber && (*tit)->GetMCS()>1) {
		//(*tit)->SetMCS((*tit)->GetMCS()-1);
	       	DEBUG_OUT << "Updating weight for " << nodeID << " streamNumber " << streamNumber << " slot " << (*tit)->GetTimeSlot() << " MCS=" << (*tit)->GetMCS() << endl;
		float rate = ((float)AdHocWiFiMac::bitrates[(*tit)->GetMCS()])/AdHocWiFiMac::bitrates[(*tit)->GetMCS()+1];
		(*tit)->SetFlowInSlot((*tit)->GetFlowInSlot()*rate);
            }
	    //newRate += (*tit)->GetFlowInSlot()*1E6;
          }
        }
	DEBUG_OUT << " updated = " << newRate << endl;
}
...
//    	rateIn = maxRateOut;
    ...
    FOR_EACH(IntSet,it,incomingTasks) {
      ...
      sendingToMeAppl->updateWeight(newRate,oldRate,streamNumber,16000/*sendingToMeAppl->channels[0]->getMsgLength()*/);

      sendingToMeAppl->streams[streamNumber]->outEdges[nodeID].rate = newRate;
      if (newRate != (unsigned long)oldRate) {
        cout << "FC: Host " << nodeID << "(" << sendingToMeNodeID << ") weight=" << weight <<
	...
	cout << "Node:" << nodeID << " Rate=" << rateIn << " prev rate= " << rateOut << " In=" <<
...
void VideoStreamAppl::Stream::queueDrop(int nodeID,unsigned long rate)
{
  ...
  if (removeSize)
	  cout << "Node:" << nodeID << " str#" << streamNumber << " chopped " << removeSize*8 << " bits from queue" << endl;
}
