...
void AdHocWiFiPhyLayer::initialize(int stage)
{
  if (stage==1) {
	...
	cModule * macM = nic->getSubmodule("mac");
	mac = check_and_cast<AdHocWiFiMac*>(macM);
	assert(mac);
  }
...
//		double centerFreq = params["centerFrequency"];
		...
		double RtsSensitivityRatio = (double) par("RtsSensitivityRatio");

		return new AdHocWiFiDecider(this, this,threshold, sensitivity, centerFreq, findHost()->getIndex(),
				coreDebug,phyOpMode,channelModel,deciderMode,payloadBerThreshold,external_PER,RtsSensitivityRatio);
	}
