...
{
public:
	GraphGenerator(GenerationParams & params, const RateCalculator & rateCalculator, unsigned int seed = 0);
	~GraphGenerator();
...
protected:
	...
	void				GenerateVerticesFromMetadata(std::string & metadataFilename, CommunicationGraph * pGraph,InterferenceGraph * pInterferenceGraph);
	...
	CommunicationRequest GenerateRequest(int requestNum, CommunicationGraph & graph, double farThreshold, bool isBelow, double demand);

	void                 TrimSpaces(std::string& str);

	bool ReadMetadataFile(std::string & metadataFilename, 
						  std::vector<int> & nVerticesInCluster,
						  std::vector<std::vector<std::tr1::tuple<double, double, double>>> & clustersTracks, 
						  std::string & errorStr);

	void GenerateCluster(std::vector<std::tr1::tuple<double, double, double>> & cluster, 
