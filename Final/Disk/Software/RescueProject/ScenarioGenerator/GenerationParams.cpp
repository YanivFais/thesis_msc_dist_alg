...
const int GenerationParams::DEFAULT_SEED_FOR_GENERATOR = 0.;
const double GenerationParams::DEFAULT_CLUSTER_RADIUS = 0.02;  
...
const int    GenerationParams::DEFAULT_PATHLOSS_MODEL = 1;
const double GenerationParams::DEFAULT_PATHLOSS_ALPHA = -1.;
const double GenerationParams::DEFAULT_PATHLOSS_BETA = -1.;
const double GenerationParams::DEFAULT_PATHLOSS_ELBIT_C = 0.;
const double GenerationParams::DEFAULT_PATHLOSS_ELBIT_H_TR = 1.;
const double GenerationParams::DEFAULT_PATHLOSS_ELBIT_H_RC = 1.;
const double GenerationParams::DEFAULT_DEMAND_SIZE = 1.;
...
GenerationParams::GenerationParams(const char * configFileName)
{
	...
	SEED_FOR_GENERATOR       = params.findInt   ("SEED_FOR_GENERATOR",DEFAULT_SEED_FOR_GENERATOR);
	CLUSTER_RADIUS           = params.findDouble("CLUSTER_RADIUS",DEFAULT_CLUSTER_RADIUS);
	...
	DEMAND_SIZE = params.findDouble("DEMAND_SIZE", DEFAULT_DEMAND_SIZE);
	...
	PATHLOSS_MODEL = params.findInt("PATHLOSS_MODEL",DEFAULT_PATHLOSS_MODEL);
	PATHLOSS_ALPHA = params.findDouble("PATHLOSS_ALPHA",DEFAULT_PATHLOSS_ALPHA);
	PATHLOSS_BETA = params.findDouble("PATHLOSS_BETA",DEFAULT_PATHLOSS_BETA);
	PATHLOSS_ELBIT_C = params.findDouble("PATHLOSS_ELBIT_C",DEFAULT_PATHLOSS_ELBIT_C);
	PATHLOSS_ELBIT_H_TR = params.findDouble("PATHLOSS_ELBIT_H_TR",DEFAULT_PATHLOSS_ELBIT_H_TR);
	PATHLOSS_ELBIT_H_RC = params.findDouble("PATHLOSS_ELBIT_H_RC",DEFAULT_PATHLOSS_ELBIT_H_RC);
	if (PATHLOSS_MODEL == 1)  // book model
	{
		double SF = NUM_STD_FOR_DECAY*SF_AFTER_BREAKPOINT;
		...
		double L_FS = 20*log10(D_BREAKPOINT) + 20*log10(FREQUENCY*1e9) - 147.5;
		PATHLOSS_BETA = L_FS - 35*log10(D_BREAKPOINT) + SF;
		PATHLOSS_ALPHA = 35;
	}
	else
	{
		if (PATHLOSS_MODEL == 2)  // Elbit model
		{
			// frequency in Elbit's model is in MHz, so multiply by 1e3 
			PATHLOSS_ALPHA = 44.9 - 6.55*log10(PATHLOSS_ELBIT_H_TR);
			PATHLOSS_BETA = 45.5 + PATHLOSS_ELBIT_C - 13.82*log10(PATHLOSS_ELBIT_H_TR) 
				+ 35.46*log10(FREQUENCY*1e3) - 1.1*PATHLOSS_ELBIT_H_RC*log10(FREQUENCY*1e3) 
				- 134.7 + 19.65*log10(PATHLOSS_ELBIT_H_TR) + 0.7*PATHLOSS_ELBIT_H_RC;
		}
		// in case it is 3 the user provides the ALPHA and BETA
	}
