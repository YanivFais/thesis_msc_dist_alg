...
void AdHocWiFiMobility::makeMove()
{
	...
	Positions positions = timedPositions[index++];
	Position position = positions.data[ID];

    Coord stepTarget(position.x,position.y);
	...
	DEBUG_MOVE("Move:" << ID << " @" << SIMTIME_DBL(simTime()) << " " << move.startPos.getX() << "," << move.startPos.getY() << " " << move.speed << endl;)

	if (timedPositions.size()>(unsigned)index+1) {
    	Positions nextPositions = timedPositions[index];
    	Position nextPosition = nextPositions.data[ID];
    	if (nextPosition.x < 0)
    		nextPosition.x = 1;
    	if (nextPosition.y < 0)
    		nextPosition.y = 1;
