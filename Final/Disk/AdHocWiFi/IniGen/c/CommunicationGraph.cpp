...
bool CommunicationGraph::InitFromFiles(const char * commGraphFileName,const char * coordinatesFileName, double T)
{
	if (ReadCoordinatesFile(coordinatesFileName, T) == false)
		return false;
...
void CommunicationGraph::calcNameTimeExtension(double T, string & fnameAtT) const
{
	char tAsString[20];
	...
	if (T2 == 0)
		sprintf(tAsString,"_%03d.000.txt",T1);
	else 
		if (T2 < 10)
			sprintf(tAsString,"_%03d.00%d.txt",T1,T2);
		else 
			if (T2 < 100)
				sprintf(tAsString, "_%03d.0%d.txt",T1,T2);
			else 
				sprintf(tAsString,"_%03d.%d.txt", T1,T2);
...
bool CommunicationGraph::WriteCoordinatesFile (const char * coordinatesFileName, double T) const
{
	...
	calcNameTimeExtension(T, fnameAtT);
...
bool CommunicationGraph::ReadCoordinatesFile(const char *coordinatesFileName, double T)
{
	string fnameAtT = coordinatesFileName;
	calcNameTimeExtension(T,fnameAtT);
